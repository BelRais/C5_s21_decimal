...
int s21_scale_equalization(s21_decimal *value_1, s21_decimal *value_2,
  ...
  if (s21_getScale(*value_1) > s21_getScale(*value_2)) {  // how are bigger
    left = value_1;
    right = value_2;
  } else {
    left = value_2;
  ...
  if (s21_getScale(*value_1) != s21_getScale(*value_2)) {  // razmer sdviga
    int sign1 = s21_getSign(*left),
        sign2 = s21_getSign(*right);  // get Sign
